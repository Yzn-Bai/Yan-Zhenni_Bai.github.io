...
------
  ...
  To simulate lane-changing behavior for both vehicle types, this study also incorporates a logit model, which takes into account the traffic occupancy rate and other latent factors influencing lane-changing decisions.
  ...
  Through these enhancements, the traditional Cell Transmission Model (CTM) is modified to better capture the unique dynamics introduced by CAVs and Dedicated Lanes (DLs), thereby improving the simulation capability of CAV DL performance in mixed traffic environments.
...
------
  ...
  The control problem is framed as a Markov Decision Process (MDP), where the state space incorporates key factors such as segment-level traffic density and CAV penetration rate, the action space represents DL configurations (such as the number of DLs), and the reward function is based on improvements in macroscopic traffic efficiency. To ensure training stability and efficiency, an early-stopping mechanism is introduced, helping to balance the trade-off between training time and convergence.
